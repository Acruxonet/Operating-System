...
ARCHIVE=${FS_ARCHIVE:-$SCRIPT_DIR/fs_solution.tar.gz}
...
TMP_DIR=$(mktemp -d /tmp/xv6-fs-solution.XXXXXX)
BUILD_LOG=/tmp/xv6-fs-build.log
TEST_LOG=/tmp/xv6-fs-test.log
cleanup() { rm -rf "$TMP_DIR"; }
trap cleanup EXIT INT TERM

echo '[1/4] Installing filesystem solution...'
for source in Makefile kernel/fcntl.h kernel/file.h kernel/fs.c kernel/fs.h kernel/param.h kernel/stat.h kernel/syscall.c kernel/syscall.h kernel/sysfile.c user/user.h user/usys.pl; do
...
cd "$REPO"
if [ "${FS_SKIP_CHECKOUT:-0}" != 1 ] && [ -d .git ]; then
  current_ref=$(git symbolic-ref --quiet HEAD 2>/dev/null || true)
  if [ "$current_ref" != refs/heads/fs ]; then
    if ! git diff --quiet || ! git diff --cached --quiet; then
      ...
      GIT_COMMITTER_EMAIL='xv6-lab-backup@localhost' \
        git stash save 'Saved automatically before filesystem lab' >/dev/null
      echo 'Existing tracked changes saved in git stash.'
    fi
    if git show-ref --verify --quiet refs/heads/fs; then
      git checkout fs >/dev/null
    else
      git checkout -b fs origin/fs >/dev/null
    fi
  fi
fi
...
echo '[2/4] Building xv6 and a fresh 70,000-block filesystem image...'
...
echo '[3/4] Running bigfile and symlinktest...'
...
def run_tests(cpu_args):
    ...
    with open(log_path, "ab") as log:
        ...
        try:
            read_until(b"$ ", 60)
            output = run_command("bigfile", 2400)
            if b"wrote 65803 blocks" not in output or b"bigfile done; ok" not in output:
                raise RuntimeError("bigfile did not verify all 65,803 blocks")
            output = run_command("rm big.file", 60)
            if b"$ " not in output:
                raise RuntimeError("could not remove big.file")
            output = run_command("symlinktest", 180)
            if b"test symlinks: ok" not in output or b"test concurrent symlinks: ok" not in output:
                raise RuntimeError("symlinktest failed")
...
print("=" * 54)
print("FILESYSTEM LAB: ALL TESTS PASSED")
print("bigfile: 65,803 blocks verified")
print("symlinktest: symlinks and concurrency OK")
